...
export function ApplicationsWorkspace({ applications }: Readonly<{ applications: ApplicationData[] }>) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-6">
      <PageHeader
        action={<Link className="motion-control inline-flex h-10 items-center gap-2 rounded-md bg-[#101318] px-4 text-sm font-bold text-white hover:bg-[#292d34]" href="/jobs/new"><Plus className="h-4 w-4 text-[#FF6B60]" />Analyze role</Link>}
        description="Keep opportunities moving with one visible owner, stage, date, score, and next action."
      ...
      <Reveal className="relative overflow-hidden rounded-lg bg-[#101318] text-white shadow-[0_20px_46px_rgba(16,19,24,0.14)]">
        ...
        <div className="relative grid grid-cols-2 lg:grid-cols-4">
          <PipelineMetric icon={Target} label="Active" value={activeCount.toString()} />
          <PipelineMetric icon={CalendarClock} label="Upcoming" value={upcomingCount.toString()} />
          <PipelineMetric icon={CheckCircle2} label="Interviews" value={interviewCount.toString()} />
          <PipelineMetric icon={BriefcaseBusiness} label="Average fit" value={averageFit === null ? "--" : `${Math.round(averageFit)}%`} />
        </div>
      </Reveal>
      ...
      <section className="app-surface overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div className="grid flex-1 gap-3 sm:max-w-[600px] sm:grid-cols-[1fr_160px]">
            <div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Search applications" className="pl-10" onChange={(event) => setQuery(event.target.value)} placeholder="Search company, role, or action" value={query} /></div>
            <div className="relative"><ListFilter className="pointer-events-none absolute left-3 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Select aria-label="Filter by status" className="pl-10" onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} value={statusFilter}><option value="all">All stages</option><option value="active">Active only</option>{STATUS_COLUMNS.map((column) => <option key={column.status} value={column.status}>{column.label}</option>)}</Select></div>
          </div>
          <div aria-label="Pipeline view" className="flex rounded-md border border-border bg-[#f7f8f9] p-1">
            <ViewButton active={view === "board"} icon={Columns3} label="Board" onClick={() => setView("board")} />
            <ViewButton active={view === "table"} icon={Table2} label="Table" onClick={() => setView("table")} />
          </div>
        </div>

        <div className="flex items-center justify-between gap-3 border-b border-border bg-[#f8f9fa] px-5 py-3 text-xs">
          <span className="font-semibold text-muted-foreground">Showing <strong className="text-foreground">{filtered.length}</strong> of {applications.length} opportunities</span>
          {(query || statusFilter !== "all") ? <button className="font-bold text-[#D9473F] hover:text-foreground" onClick={() => { setQuery(""); setStatusFilter("all"); }} type="button">Clear filters</button> : null}
        </div>
          ...
          return (
            <section className="w-[282px] shrink-0 rounded-lg border border-border bg-[#f7f8f9]" key={column.status}>
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                <div className="flex items-center gap-2"><span className={cn("h-2 w-2 rounded-full", statusDot(column.status))} /><h2 className="text-sm font-bold text-foreground">{column.label}</h2></div>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1040px] border-collapse text-left text-sm">
        <thead className="bg-white text-[10px] uppercase text-muted-foreground"><tr><th className="px-5 py-3 font-bold">Opportunity</th><th className="px-4 py-3 font-bold">Stage</th><th className="px-4 py-3 font-bold">Deadline</th><th className="px-4 py-3 font-bold">Fit</th><th className="px-4 py-3 font-bold">Applied</th><th className="px-4 py-3 font-bold">Interview</th><th className="px-4 py-3 font-bold">Next action</th><th aria-label="Open" className="w-12 px-4 py-3" /></tr></thead>
        <tbody>{applications.length ? applications.map((application) => <ApplicationRow application={application} key={application.id} />) : <tr><td className="px-5 py-10 text-center text-muted-foreground" colSpan={8}>No applications match the current filters.</td></tr>}</tbody>
      </table>
    </div>
  );
}
...
function ApplicationRow({ application }: Readonly<{ application: ApplicationData }>) {
  return (
    <tr className="border-t border-border transition hover:bg-[#f8f9fa]">
      ...
      <td className="max-w-[300px] truncate px-4 py-4 text-muted-foreground">{application.next_action ?? "Not set"}</td>
      <td className="px-4 py-4"><Link aria-label={`Open ${application.company}`} className="grid h-8 w-8 place-items-center rounded-md border border-border text-muted-foreground hover:border-[#FF5A4E] hover:text-[#D9473F]" href={`/applications/${application.id}`}><ArrowRight className="h-4 w-4" /></Link></td>
    </tr>
  );
}
  ...
  return (
    <Link className="group block overflow-hidden rounded-lg border border-border bg-white shadow-[0_1px_2px_rgba(16,19,24,0.04)] transition hover:-translate-y-0.5 hover:border-[#b7bdc5] hover:shadow-[0_12px_24px_rgba(16,19,24,0.08)]" href={`/applications/${application.id}`}>
      ...
      <div className="p-4">
        ...
        <div className="mt-4 border-t border-border pt-3"><p className="text-[10px] font-bold uppercase text-muted-foreground">Next action</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-[#4d545e]">{application.next_action ?? "Define the next action"}</p></div>
        <div className="mt-3 flex items-center justify-between gap-2 text-[10px] font-semibold text-muted-foreground"><span className="flex items-center gap-1"><CalendarClock className="h-3 w-3" />{formatDate(application.deadline)}</span><span>{application.interview_prep_id ? "Prep ready" : "No prep"}</span></div>
      </div>
    </Link>
  );
}
...
function StatusBadge({ status }: Readonly<{ status: ApplicationStatus }>) {
  return <span className={cn("inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[10px] font-bold uppercase", statusTone(status))}><span className={cn("h-1.5 w-1.5 rounded-full", statusDot(status))} />{status}</span>;
}
