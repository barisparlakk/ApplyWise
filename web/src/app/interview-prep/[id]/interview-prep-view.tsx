...
export function InterviewPrepView({ apiBaseUrl, initialPrep }: InterviewPrepViewProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] space-y-6">
      <PageHeader
        action={<Link className="motion-control inline-flex h-10 items-center gap-2 rounded-md border border-border bg-white px-3.5 text-xs font-bold text-foreground hover:border-[#FF5A4E] hover:text-[#D9473F]" href={`/jobs/${prep.application.job_post_id}/analysis`}><ArrowLeft className="h-4 w-4" />Back to analysis</Link>}
      ...
      <nav aria-label="Interview prep sections" className="flex gap-1 overflow-x-auto border-b border-border">
        {[["#scripts", "Scripts"], ["#technical", "Technical"], ["#behavioral", "Behavioral"], ["#star", "STAR"], ["#drills", "Weak-area drills"]].map(([href, label]) => <a className="motion-control whitespace-nowrap border-b-2 border-transparent px-3 py-3 text-sm font-bold text-muted-foreground hover:border-[#FF5A4E] hover:text-foreground" href={href} key={href}>{label}</a>)}
      </nav>

      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_370px]">
        <main className="min-w-0 space-y-6">
          <section className="space-y-6" id="scripts">
            <ScriptSection icon={UserRound} isBusy={isBusy("english_self_introduction")} onRegenerate={() => void regenerateSection("english_self_introduction")} script={content.english_self_introduction} title={SECTION_LABELS.english_self_introduction} />
            <ScriptSection icon={Code2} isBusy={isBusy("project_explanation_script")} onRegenerate={() => void regenerateSection("project_explanation_script")} script={content.project_explanation_script} title={SECTION_LABELS.project_explanation_script} />
            <div className="grid gap-6 lg:grid-cols-2">
              <ScriptSection icon={Building2} isBusy={isBusy("why_this_company")} onRegenerate={() => void regenerateSection("why_this_company")} script={content.why_this_company} title={SECTION_LABELS.why_this_company} />
              <ScriptSection icon={Target} isBusy={isBusy("why_this_role")} onRegenerate={() => void regenerateSection("why_this_role")} script={content.why_this_role} title={SECTION_LABELS.why_this_role} />
            </div>
          </section>

          <div id="technical"><QuestionSection icon={BrainCircuit} isBusy={isBusy("technical_questions")} onRegenerate={() => void regenerateSection("technical_questions")} questions={content.technical_questions} title={SECTION_LABELS.technical_questions} /></div>
          <div id="behavioral"><QuestionSection icon={MessageSquareText} isBusy={isBusy("behavioral_questions")} onRegenerate={() => void regenerateSection("behavioral_questions")} questions={content.behavioral_questions} title={SECTION_LABELS.behavioral_questions} /></div>
        </main>
        ...
        <aside className="space-y-6 xl:sticky xl:top-24 xl:self-start">
          ...
          <div id="drills"><QuestionSection compact icon={ScanSearch} isBusy={isBusy("weak_area_drill_questions")} onRegenerate={() => void regenerateSection("weak_area_drill_questions")} questions={content.weak_area_drill_questions} title={SECTION_LABELS.weak_area_drill_questions} /></div>
        </aside>
      </div>
    </div>
  );
...
function StarSection({ isBusy, onRegenerate, templates }: Readonly<{ isBusy: boolean; onRegenerate: () => void; templates: InterviewPrepStarTemplate[] }>) {
  return (
    <Reveal className="app-surface overflow-hidden">
      <SectionHeader icon={Star} isBusy={isBusy} onRegenerate={onRegenerate} title={SECTION_LABELS.star_answer_templates} />
      <div className="divide-y divide-border">
          ...
          <details className="group" key={template.prompt} open={index === 0}>
            ...
            <div className="border-t border-border bg-[#f8f9fa] p-5">
              <dl className="relative space-y-4 before:absolute before:bottom-4 before:left-[13px] before:top-4 before:w-px before:bg-border">
                <StarItem label="S" name="Situation" value={template.situation} />
                <StarItem label="T" name="Task" value={template.task} />
                <StarItem label="A" name="Action" value={template.action} />
                <StarItem label="R" name="Result" value={template.result} />
              </dl>
...
function SectionHeader({ icon: Icon, isBusy, onRegenerate, title }: Readonly<{ icon: LucideIcon; isBusy: boolean; onRegenerate: () => void; title: string }>) {
  return (
    <div className="flex items-center justify-between gap-4 border-b border-border px-5 py-4 sm:px-6">
      ...
      <Button aria-label={`Regenerate ${title}`} disabled={isBusy} onClick={onRegenerate} size="icon" title={`Regenerate ${title}`} type="button" variant="ghost">{isBusy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}</Button>
    </div>
  );
}
...
function EvidenceList({ evidence }: Readonly<{ evidence: string[] }>) {
  ...
  return (
    <details className="group/evidence mt-5 border-t border-border pt-3">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-[10px] font-bold uppercase text-muted-foreground"><span className="flex items-center gap-2"><BookOpenCheck className="h-3.5 w-3.5 text-[#167D87]" />Grounded evidence ({evidence.length})</span><ChevronDown className="h-3.5 w-3.5 transition group-open/evidence:rotate-180" /></summary>
