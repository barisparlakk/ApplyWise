  ...
  return (
    <div className="w-full">
        ...
        <div className="space-y-3">
          {googleEnabled ? (
            <button className="motion-control flex h-12 w-full items-center justify-between rounded-md border border-border bg-white px-4 text-sm font-bold text-foreground shadow-sm hover:border-[#a8afb8] hover:bg-[#fafafa] disabled:opacity-60" disabled={pendingProvider !== null} onClick={() => void continueWith("google")} type="button">
              <span className="flex items-center gap-3"><span className="grid h-6 w-6 place-items-center rounded border border-border text-xs font-bold text-[#4285F4]">G</span>{t("Continue with Google")}</span>
          ...
          {githubEnabled ? (
            <button className="motion-control flex h-12 w-full items-center justify-between rounded-md border border-border bg-white px-4 text-sm font-bold text-foreground shadow-sm hover:border-[#a8afb8] hover:bg-[#fafafa] disabled:opacity-60" disabled={pendingProvider !== null} onClick={() => void continueWith("github")} type="button">
              <span className="flex items-center gap-3"><GitFork className="h-5 w-5" />{t("Continue with GitHub")}</span>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </button>
          ) : null}
        </div>
      ...
      {emailEnabled && socialLoginEnabled ? (
        <div className="my-6 flex items-center gap-3 text-[11px] font-semibold uppercase text-muted-foreground"><span className="h-px flex-1 bg-border" />{t("Development access")}<span className="h-px flex-1 bg-border" /></div>
      ) : null}

      {emailEnabled ? (
        <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); void continueWith("email", { email }); }}>
          <label className="text-sm font-semibold text-foreground" htmlFor="email">{t("Email")}</label>
          ...
          <button className="motion-control flex h-11 w-full items-center justify-center gap-2 rounded-md bg-[#101318] px-4 text-sm font-bold text-white hover:bg-[#282c34] disabled:opacity-60" disabled={pendingProvider !== null} type="submit">{t("Continue with email")} <ArrowRight className="h-4 w-4 text-[#FF6B60]" /></button>
        </form>
      ) : null}

      <p aria-live="polite" className="mt-5 min-h-5 text-xs text-muted-foreground">{pendingProvider ? t("Opening secure sign-in...") : t("Secure access with account-scoped data.")}</p>
      <p className="mt-5 border-t border-border pt-5 text-xs leading-5 text-muted-foreground">{t("By continuing, you agree to the")} <a className="font-bold text-foreground hover:text-primary" href="/terms">{t("terms")}</a> {t("and acknowledge the")} <a className="font-bold text-foreground hover:text-primary" href="/privacy">{t("privacy notice")}</a>.</p>
    </div>
  );
}
