...
export function DeleteAccountButton() {
  ...
  async function deleteAccount() {
    ...
    if (!response.ok) {
      setErrorMessage(t((await apiError(response, t("Account deletion failed"))).message));
      setIsDeleting(false);
      return;
    }
  ...
  return (
    <motion.div animate={{ opacity: 1, y: 0 }} className="max-w-md rounded-md border border-[#f0b5b0] bg-white p-4 shadow-sm" initial={{ opacity: 0, y: 4 }}>
      <p className="flex items-center gap-2 text-sm font-bold text-[#A63832]"><AlertTriangle className="h-4 w-4" />{t("Permanently delete this account?")}</p>
      <p className="mt-2 text-sm leading-6 text-muted-foreground">
        {t("This removes your profile, CVs, repository analyses, jobs, applications, roadmaps, and interview preparation. This action cannot be undone.")}
      </p>
