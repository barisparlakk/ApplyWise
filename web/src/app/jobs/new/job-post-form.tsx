      ...
      <Reveal className="relative overflow-hidden rounded-lg bg-[#101318] text-white shadow-[0_20px_46px_rgba(16,19,24,0.14)]">
        ...
        <div className="relative grid lg:grid-cols-[1fr_420px]">
          <div className="p-6 sm:p-8">
            ...
            <h2 className="mt-4 text-2xl font-bold">{isReady ? t("Source is ready for analysis") : t("Paste the complete role listing")}</h2>
            <p className="mt-2 max-w-xl text-sm leading-6 text-white/[0.55]">{t("Include responsibilities, requirements, company context, location, and language expectations when available.")}</p>
          </div>
          <div className="border-t border-white/[0.10] p-6 sm:p-8 lg:border-l lg:border-t-0">
            <div className="flex items-end justify-between gap-4">
              <div><p className="text-[10px] font-bold uppercase text-white/[0.45]">{t("Source depth")}</p><p className="mt-2 text-4xl font-bold">{trimmedLength}</p></div>
              <span className="text-xs font-semibold text-white/[0.48]">{t("characters captured")}</span>
            </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <Reveal className="app-surface overflow-hidden" delay={0.04}>
          <div className="border-b border-border p-5 sm:p-6">
            <SectionHeading action={<span className="data-chip"><ClipboardPaste className="mr-1.5 h-3.5 w-3.5" />{t("Manual source")}</span>} description={t("The full listing produces a more reliable structured breakdown.")} title={t("Job description")} />
          </div>
          ...
          <div className="flex flex-col gap-4 border-t border-border bg-[#f8f9fa] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div aria-live="polite" className="text-sm text-muted-foreground">
              <span className="flex items-center gap-2 font-semibold"><span className={isReady ? "h-2 w-2 rounded-full bg-[#2BC3CE]" : "h-2 w-2 rounded-full bg-[#F0A13A]"} />{isReady ? t("Ready to extract role signals") : t("{count} more characters needed", { count: Math.max(0, 40 - trimmedLength) })}</span>
            ...
            <Button disabled={!isReady || state === "analyzing"} onClick={() => void analyzeJobPost()} type="button">
              ...
              {state === "analyzing" ? t("Analyzing role") : t("Analyze role")}
            </Button>
          </div>
        </Reveal>
